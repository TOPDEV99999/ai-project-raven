...
static mut MIC_STOP_FLAG: Option<Arc<AtomicBool>> = None;
...
pub fn start_mic_capture(callback: JsFunction) -> Result<bool> {
    ...
    let stop_flag = Arc::new(AtomicBool::new(false));
    unsafe {
        MIC_STOP_FLAG = Some(stop_flag.clone());
    }

    MIC_CAPTURING.store(true, Ordering::SeqCst);

    thread::spawn(move || {
        if let Err(e) = wasapi::mic_capture_loop(stop_flag, move |chunk| {
            tsfn.call(chunk, ThreadsafeFunctionCallMode::NonBlocking);
        }) {
            eprintln!("[WASAPI-Mic] Capture error: {:?}", e);
        }
...
pub fn stop_mic_capture() -> bool {
    unsafe {
        if let Some(ref flag) = MIC_STOP_FLAG {
            flag.store(true, Ordering::SeqCst);
            MIC_STOP_FLAG = None;
            return true;
        }
